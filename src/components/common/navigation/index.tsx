  ...
  return (
    <header className={clsx("py-4 text-white w-full z-10", customClass)}>
      <nav className="w-full md:max-w-7xl mx-auto flex justify-between items-center px-5">
		...
		<button className="font-semibold text-brand-grey/200 focus:outline-none">
			Genre
		</button>
		{isGenreOpen && (
			<div className="absolute  bg-white w-max rounded-md shadow-lg grid grid-cols-3">
			{genresData?.genres.map((item)=>(
			<Link to="#" className="text-center block px-4 py-2 text-brand-grey/200 hover:bg-gray-100 hover:text-brand-grey/500">
				{item.name}
			</Link>
			))}
			</div>
